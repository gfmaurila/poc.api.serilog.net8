...
namespace poc.api.serilog.net8.EndPoints;
public static class ProdutosEndpoints
{
    public static void RegisterProdutosEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapGet("/api/produto", async (IProdutoService _service, ILogger<Program> logger) =>
        {
            ...
            if (produto is null)
            {
                logger.LogWarning("Nenhum produto encontrado");
                return Results.NotFound();
            }

            logger.LogInformation("Produtos encontrados: {ProdutoCount}", produto.Count());
